...
class Tile:
    ...
    def __init__(self, data, detector="sift"):
        ...
        self.source = data 
        self.imdata = data if isinstance(self.source, np.memmap) else self.load_imdata() 
    ...
    def draw_memmap(self, others=None):
        ...
        tiles = [t for t in [self] + others if t.placed]
        print("In draw memmap tile count {}".format(len(tiles)))
        ys = []
        xs = []
        for tile in tiles:
            y1, x1, y2, x2 = tile.bounds(as_int=True)
        ...
        data = np.memmap('large_matrix_{}_{}_{}.dat'.format(shape[0], shape[1], shape[2]), dtype='int8', mode='w+', shape=shape)
        print("Saving large matrix of size: {}".format(shape))
