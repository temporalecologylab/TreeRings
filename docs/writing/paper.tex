...
Solving this in a time-efficient manner involves two stages. We provide a complete glossary of notation used to describe the methods, see Table \ref{tab:math_notation}.
...
From the first coordinate captured $(X_0, Y_0, Z_0)$, multiple images are taken at different heights (saved as a vector $\boldsymbol{Z_{\text{samples}}}$). %emw9Mar: I think we need to be a little more careful in defining the math as we introduce each new term. One way to start on this would be to make a supplemental table with ALL the math we introduce and a brief description of it. If you can work that up then we could meet and fix this quickly I suspect. 
The difference in height between each image are equally spaced, and are described by the relative distance to $Z_0$ in the vector $\boldsymbol{\delta}$. 
We found that it is important to have a distance of no more than 0.1 mm between each image in the stack to maximize the likelihood of an in-focus image at height $Z_{focused}$.
...
The second stage of our time-efficient focusing procedure is uses a feedback loop to update the initial $Z_0$ at each image location $(X_k, Y_k, Z_k)$. 
...
Rather than increasing this height range, a greater alignment error can be managed by adjusting the starting image height until $Z_{focused}$ is at the center index of $\boldsymbol{Z_{samples}}$. 
The likelihood of an adjacent target
$(X_{k+1}, Y_{k+1})$ containing an in-focus image is highest in this scenario. 
A negative feedback PID control algorithm is used to adjust the initial height of each image, with $\alpha_k$, as the camera navigates the sample \citep{odwyer_summary_2000}. 
...
We tested a scanned cookie of size (HEIGHT)X(WIDTH) (WAITING FOR SANDY) pixels from TIM with the R-CNN presented by \citeauthor{polacek_automation_2023} on two servers. The first being the high-performance UBC ARC Sockeye which completed the task in 15 minutes. As many do not have access to such powerful computing, we also tested
