...
For the computer, we implemented all of the software control, image stitching, and graphical user interface (GUI) to run on an NVIDIA Jetson Orin Nano 8GB (Jetson edge computer), which also connects to the camera via a MIPI cable. % amf28jan2026 The Jetson Orin Nano 8GB is it's complete product name
...
In such cases, we developed a particle smearing method which add features to the sample, aiding the stitching process. 
...
%amf28jan2026 -- FLAGGING flagging
...
No extra tools are needed for this method--a fingertip dipped in the powder is sufficient. T powder shehould be smeared across the sample's sanded surface, smoothing any clumps of powder in the process. 
When applied correctly, trace amounts of silicon carbide will be embedded in the surface roughness of the sample which can not be seen without microscope magnification. Images of a core before and after powder smearing can be seen in Figure \ref{fig:core_smearing} 
Most of the silicon carbide can be removed after scanning is complete by blowing compressed air on the sample. 
...
Samples larger than this limit can still be scanned, but TIM saves them as an uncompressed binary NumPy memory-mapped array file. Currently, NumPy files are not supported by standard image viewers like ImageJ and require custom tools to work with. %emw17Jan2026 -- any chance we can give an (e.g., THIS tool) to the end here? %amf28Jan2026 -- This would be some GUI wrapping NumPy functions which are made to edit the NumPy files. I'm certain this could be done but it doesn't really seem worth it. I imagine in order for it to actually be useful, it would need to offer complete annotation functionality for marking and be user friendly in a way that I wouldn't be able to churn out quickly.
...
%amf28Jan2026 -- FLAGGING quick review
