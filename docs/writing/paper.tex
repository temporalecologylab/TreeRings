...
For the computer, we implemented all of the software control, image stitching, and graphical user interface (GUI) to run on an NVIDIA Jetson Orin Nano (Jetson edge computer), which also connects to the camera via a MIPI cable. %emw17Jan2026 -- given we talk about RAM later, I think perhaps here we should give a few details on the specs -- so in the () we write Jetson edge computer with XX RAM etc?
...
In such cases, we developed two methods that add features to the sample to aid the stitching process. %emw17Jan2026 -- we say two methods but we introduce just one ... ? We should introduce the other one or update these to be one section that introduces 'a method.'
