...
%DIF ADD paper_anon.tex         Sun Aug 16 23:11:57 2026
...
1.  Studies of tree rings from climatology to ecology have become increasingly important with anthropogenic climate change, with a need for research across more locations and species. Meeting the increasing demand requires methods to image samples rapidly and efficiently, while also producing high quality images. % Getting rid of open-source as CaptuRING is cheap but GigaPixel is expensive and not open source
...
These savings combined with not needing to purchase a professional stitching software license (such as PTGui from New House Internet Services BV, Rotterdam, NL, which is the suggested software to use with CaptuRING) makes TIM a more economical option than most products \DIFdelbegin \DIFdel{current available. }\DIFdelend \DIFaddbegin \DIFadd{currently available. }\R{line_change_1}
\DIFaddend Additionally the design is open-source and open-hardware allowing for contributors to add new sensors or alter the control methods. 
...
\begin{figure}
  ...
  \caption{Complete assembly of TIM, shown with a few smaller cookies ready to be scanned. All of the 3D printed parts are available for download on the \href{https://doi.org/10.60705/3DPX/21561.3}{NIH 3D printing repository} and the software is  
...
The general guidelines used for preparing cookies and cores applies---requiring samples to be sanded with incrementally increasing sand paper grit. Samples must be uniformly sanded and have \DIFaddbegin \DIFadd{a }\DIFaddend planar surface for scanning \citep{speer_fundamentals_2010} \DIFaddbegin \R{line_change_2}\DIFaddend .
...
\DIFaddend Small differences in plane angle can be corrected using the 3D printed leveling table we designed (Figure \ref{fig:ideal_levelling}). \DIFaddbegin \todo{Make figure appear after reference.}
\begin{figure}
  \centering
...
If the Golden Section Search algorithm \DIFdelbegin \DIFdel{only finds images with a sharpness metric lower }\DIFdelend \DIFaddbegin \DIFadd{cannot find an image with a higher sharpness metric }\DIFaddend than this threshold, it is safe to assume the sample is no longer in the frame. 
\DIFdelbegin \DIFdel{The same threshold was used across all cores across 5 species that }\DIFdelend %DIF >  Clarification comments 6
\DIFaddbegin \R{line_change_4} \DIFadd{Among the six conifer species }\DIFaddend we have scanned, \DIFdelbegin \DIFdel{but tuning may be required to prevent false positives}\DIFdelend \DIFaddbegin \DIFadd{the sharpness metric did not vary significantly when in-focus, allowing for the multiple species to be scanned in the same batch with only one core-edge threshold}\DIFaddend .
...
If the new image has at least a sharpness metric of 95\% of its previously imaged neighbor, we choose not to run the autofocusing \DIFdelbegin \DIFdel{loop}\DIFdelend \DIFaddbegin \DIFadd{procedure}\DIFaddend . If the prepared surface of the core is perfectly planar to the table that TIM is installed on, it is possible to only autofocus once for tens of images.
\DIFdelbegin \DIFdel{With the samples we prepared, there usually needed to be an autofocusing procedure }\DIFdelend \DIFaddbegin \DIFadd{When TIM encounters a new image that has a focus metric lower than it's previous neighbor, it will run the autofocusing procedure.
%DIF > % Addressing reviewer comments 1 
}\R{autofocusing_1} \DIFadd{This ensures that all neighboring images have functionally the same sharpness, while minimizing the time spent on autofocusing. 
In the case that the sample is not aligned well to the camera, the autofocusing procedure will be run more frequently. 
...
Samples that have visible scoring marks from sanding, non planar surfaces, or other major surface irregularities have difficulties scanning with or without adaptive autofocusing. These samples simply require more attentive sanding to ensure a blemish free, planar surface for scanning.    
}\R{autofocusing_2} %DIF > % End reviewer comments 1
\DIFaddend 
...
\DIFdelbegin \DIFdel{We found some samples were impossible to scan without additional preparation. The most common scenario leading to an incomplete scan was due to regions of the core having few distinguishable SIFT features . 
Samples often begin to lack features when focusing on }\DIFdelend %DIF >  Addressing reviewer comments 5
\DIFaddbegin \R{particle_smearing_1}
\DIFadd{Sanding samples in bulk inherently led to a few outlier samples having blemishes or imperfections on the scanning surface. We developed a benchtop method to circumvent the need to resand these samples for logistic convenience. 
Additionally, this method aids stitching images in regions of few distinguishable features despite effective sanding, primarily in regions of }\DIFaddend lightly-colored sapwood\DIFdelbegin \DIFdel{, but this problem may also occur where there are sanding blemishes or other imperfections on the scanning surface. In }\DIFdelend \DIFaddbegin \DIFadd{. %DIF >  Betula / Populus?  
For }\DIFaddend such cases, we developed a particle smearing method which \DIFdelbegin \DIFdel{adds features to the }\DIFdelend \DIFaddbegin \DIFadd{embeds particles in the prepared surface of the }\DIFaddend sample, thus aiding the stitching process.
\DIFaddbegin \DIFadd{Particle smearing does not need to be done to every sample and is best reserved for samples that have proven to be difficult to stitch. We found this method did not interfere with the measurements of annual growth rings. Measurement of other anatomical measurements such as vessel counting will likely be affected. 
}\R{particle_smearing_2}
%DIF >  End reviewer comments 5
\DIFaddend 

\subsubsection{Particle Smearing}

\DIFdelbegin \DIFdel{The ideal method for adding }\DIFdelend \DIFaddbegin \DIFadd{Particle smearing is an effective technique for adding high contrast }\DIFaddend features to the \DIFdelbegin \DIFdel{sample is to smear small particles with contrasting color compared to }\DIFdelend \DIFaddbegin \DIFadd{prepared surface of a sample by embedding small particles into }\DIFaddend the surface of the sample. 
...
\DIFaddbegin \R{llm_rewording_1}
\DIFaddend We wrote all the initial code for our first fully functioning prototype version of TIM, but used ChatGPT-4.0 to generate \DIFdelbegin \DIFdel{some
code in improving several aspects}\DIFdelend \DIFaddbegin \DIFadd{several improvements}\DIFaddend .
...
Other small snippets of code were generated to select pixels in specific regions of images. All code that was generated was reviewed for functionality and \DIFdelbegin \DIFdel{correctness before codebase integration and is noted in the code with comments. 
}\DIFdelend \DIFaddbegin \DIFadd{identified by comments in the source files. 
}\R{llm_rewording_2}
\DIFaddend 
...
\DIFaddbegin \R{usage_1}
\DIFadd{TIM has been a useful tool for scanning tree ring samples for our lab. We have scanned approximately 1,250 cores and 100 cookies with TIM. These samples included six different conifer species namely }\emph{\DIFadd{Pseudotsuga menziesii}}\DIFadd{, }\emph{\DIFadd{Tsuga heterophylla}}\DIFadd{, }\emph{\DIFadd{Tsuga mertensiana}}\DIFadd{, }\emph{\DIFadd{Abies amabilis}}\DIFadd{, }\emph{\DIFadd{Chamaecyparis nootkatensis}} \DIFadd{and }\emph{\DIFadd{Thuja plicata}}\DIFadd{. 
For our core samples, we prepared between one and three batches of 24 cores to be scanned a day over the course of three months. Many of the batches were scanned overnight. We found that the Jetson computer would occasionally crash due to unexpected memory limits if not restarted once a day. This issue was resolved by restarting the computer before starting a large batch of samples. 
}\R{usage_2}
%DIF > % End reviewer comments 2

\DIFaddend TIM is capable of producing extremely high resolution scans. We calculated the maximum dots per inch (DPI) at 21\DIFdelbegin \DIFdel{500 }\DIFdelend \DIFaddbegin \DIFadd{,140 }\DIFaddend of individual images captured from the system using a microscopy slide scale (with 0.01 mm graduations, Figure \ref{fig:dpi_measurements}), and developed a Python tool to replicate this for varied lens focal lengths. When downscaling the final stitch, 
...
To fit into the maximum 2.5 GB limits of a TIFF file, the maximum diameter of a cookie scanned at 30\% of maximum resolution is limited to approximately 130 mm. This maximum cookie diameter is significantly smaller than the maximum core sample length. \DIFdelbegin \DIFdel{At maximum resolution , the file size limit 
constrains core samples to approximately }\DIFdelend \DIFaddbegin \R{max_length} \DIFadd{Cores can be scanned at maximum resolution up to }\DIFaddend 1 \DIFdelbegin \DIFdel{500 mm in length }\DIFdelend \DIFaddbegin \DIFadd{meter which can be collected from the longest increment borer available on the market }\DIFaddend (though a larger ACRO frame than shown here would be needed to support this sample length). 
...
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \begin{subfigure}{.5\textwidth}
    \centering
    \includegraphics[height=\linewidth]{../graphs/core_scanning_time.png}
    \caption{\DIFaddFL{Core Scanning Time}}
  ...
  \end{subfigure}
  \DIFaddendFL \caption{Time to digitize a sample is dependent on the sample's surface area and the desired final resolution.
...
TIM's design minimizes the barriers to build an affordable, efficient and high-resolution image capture system. TIM does not compromise scan quality and can scan both cookies and cores. We have measured tree ring widths using the digitized samples from TIM using CooRecorder as well as ImageJ \citep{schneider_nih_2012}.
...
A useful addition to TIM would be to automatically detect tree-ring widths or other metrics while capturing images using computer vision models. We expect this may be possible as these methods improve in computational efficiency and accuracy. Currently, we found that tree-ring detection by a recent machine learning algorithm  \citep[R-CNN presented by][]{polacek_automation_2023} for a scanned core (23294 x 897 pixels) took 15 minutes on a high-performance cluster (UBC ARC Sockeye) and 3.5 hours on a smaller server (RTX A6000 GPU server). 
