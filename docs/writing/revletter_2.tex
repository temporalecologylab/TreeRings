...
We agree that stitched images are necessary to evaluate the image quality. Thus, we have added full-resolution scans for multiple species in the supplementary material  (Table \ref{tab:scans}). We reconciled the inconsistent values in submission abstract. \\

We now provide an estimate of the effective optical resolution using a 1 mm stage micrometer with 0.01 mm graduations. A cropped image of the stage micrometer taken by TIM is shown below in Figure \ref{fig:stage_micrometer} (this image and Fig. \ref{fig:stage_micrometer_4_graduations} are both now provided in the supplement). \\

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{../content/stage_micrometer_cropped.png}
    \caption{An image from TIM cropped to the extent of a 1 mm stage micrometer with 0.01 mm graduations.}
    \label{fig:stage_micrometer}
\end{figure}

When the image is cropped further to focus on four graduations (Figure \ref{fig:stage_micrometer_4_graduations}), it is possible to count about 4 pixels between each graduation mark. This indicates that we are able to distinguish features that are less than 10 microns. \\

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{../content/stage_micrometer_cropped_4_graduations.png}
    \caption{An image zoomed into four graduations of the aforementioned stage micrometer.}
    \label{fig:stage_micrometer_4_graduations}
\end{figure}
...
We have also added specificity into the use case of the particle smearing method between \lr{particle_smearing_1} and \lr{particle_smearing_2}. \\
...
We have updated the manuscript to outline this between \lr{usage_1} and \lr{usage_2}. \\
...
%emw28Aug -- check my edits are correct!!!
We appreciate the reviewer's comment and understand their concerns. We agree that many samples will not need additional surface treatment to be successfully stitched. We have thus changed the sample used in Figure 5 (main manuscript text) to be a harder-to-scan sample, more representative of a sample which would need surface treatment. Additionally, we have added specificity into the use case of the particle smearing method between \lr{particle_smearing_1} and \lr{particle_smearing_2}. This includes highlighting that most samples should not need this approach. \\
...
We appreciate the concern of Reviewer 1 and have clarified how autofocusing every few frames affects the final image sharpness between \lr{autofocusing_1} and \lr{autofocusing_2}. This clarification is quoted below. \\
...
We agree with the reviewer that a scale reference on the scan is necessary to verify the DPI. To address this, we have provided scans with a few different scale references. These include a scan of a caliper with millimeter graduations, a cookie scan with a paper ruler on its surface, a photo of a 1 mm stage micrometer with 0.01 mm graduations, and digital scale bars on each core scan. We do not provide physical references on top of cores because of the high-resolution and precise focusing of TIM---the slight addition of a change in depth from a paper scale bar causes significant blurring in the images. Cookies have enough surface area for a millimetric reference to be placed on the surface while reserving a useful portion of the sample unaffected by the blurring caused by the physical scale. \\% Scanning a sample with a physical scale bar in the image is non-trivial due to the camera's narrow focal depth.
...
More dramatic stains would not affect the imaging and stitching contrast settings. We chose to use silicon carbide as we found simple staining techniques did not resolve the stitching issue.\\ %emw28Aug -- I changed this so it jives with what we say in the main text now. You could also say at the end here: We now explain this in more detail in this section, between \lr{particle_smearing_1} - \lr{particle_smearing_2}. 
...
We thank the reviewer for their enthusiasm about the potential for ring and feature detection. We are hopeful that future methods for ring and other feature identification are developed and can be integrated with TIM through its open-source design. \\ %emw28Aug -- check my edits

\emph{Abstract point 3: It’s a little unclear with the current notation what “21 140 DPI” means – should it be 21,140 DPI? Also, one version of your abstract indicates 0.83 pixels per micron and the other 1.2 microns per pixel. Similarly for Point 3, is the value \$3,000 USD? The problem seems to be a space has been introduced to replace the thousand separators: L54 – 6 400 DPI rather than 6,400 DPI. See L67, L69, L88, L89… etc. throughout the manuscript.}\\
...
We appreciate the positive comments and the acknowledgement of the potential of our tool. We hope other groups will build and use TIM or develop further on its design. \\ %emw28Aug -- check my edits
