...
% emwAug18 - you do not need to include the editor's comments (above, they are probably boilerplate), but I think it fine to have it. But you MUST include ALL the comments from the associate editor and reviewer. ALL of them (otherwise it looks like we are hiding something). I added in the associate's editors and some of the missing ones from R1 and R2, but you need to work on the others fully. (I might just start a new file copy-and-pasting the comments and then you can grab the responses and add them, but up to you.) 
...
\todo{scan full resolution samples} % emwAug18 - more active voice! Also, I would cite one of the images in the main text and provide a complete list of them in the supp PDF. I will try to flag a spot in the main text to do this... 
We agree that stitched images are necessary to evaluate the image quality. Thus, we have added full-resolution scans for multiple species in the supplementary material.\\
...
% emwAug18 - here's a good spot to not bury the changes, see my edits below: 
...
% emwAug18 - I think you can explain the below better, I don't even know what you're saying. Or you could cut it; I think maybe it is explained well enough above? 
Capturing sequential frames without autofocusing indicates that the focus metric has not deviated from the past frame by more than 5\%, depending on configurable parameters. Therefore, the sharpness of the images are functionally identical when multiple frames are captured without refocusing the camera. \\
...
these low cost cameras.} % emwAug18 - again, I am adding in part of what you did not ... but check!
...
We appreciate the concern of Reviewer 1 and have clarified how autofocusing every few frames affects the final image sharpness between \lr{autofocusing_1} and \lr{autofocusing_2}.\\ % emwAug18 - I would QUOTE this text from the main text here ... \begin{quote}  TEXT ... \end{quote} 
% emwAug18 -- if you don't quote it then change above where I tell the editor we quote it ... 
...
We thank the reviewer for pointing out that the maximum core length exceeds practical dimensions of increment borers. We have updated the text to clarify this on \lr{max_length}.\\ % emwAug18 - nice. Replies like this are perfect...
...
% emwAug18 -- just as an example I pulled out the first small comment below to show how ALL of these should be addressed eventually (aka soon).
...
\emph{
    \begin{itemize}
        \item L96: Change “current” to currently % emwAug18 - see above.
        \item L129: Should read “…and have a planar surface for…”
        \item L137: Small point admittedly, but conventionally the reference to a figure should appear before the figure itself occurs. Same for Figure 3 which appears before the reference to it. % emwAug18 -- there is a ton online on trying to do this ... I think you may need to use floats and h!  but I would set a time limit on this (20 minutes) and then give up if this ends up being a pain. Then you can just say -- we have worked to re-position the images but found it difficult to place the image exactly before the text without causing other formatting issues. If our article were accepted, we certainly would work with the journal typesetter to make sure the images are well placed relative to the text where they are mentioned. 
