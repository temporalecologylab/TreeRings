...
This time saving feature can also be turned off, forcing the scanning procedure to include a Golden Section Search autofocusing procedure for every image. 
With the samples we prepared, there usually needed to be an autofocusing procedure for every 5-10 images while maintaining high image sharpness throughout the entire sample. 
...
\begin{figure}[htbp] 
  ...
  \centering 
  \includegraphics[width=\linewidth]{../graphs/core_scanning_time.png} 
...
\end{subfigure} 
  ...
  \begin{subfigure}{0.49\textwidth} 
  \centering 
  \includegraphics[width=\linewidth]{../graphs/cookie_scanning_time.png} 
  \caption{Cookie Scanning Time} 
  \label{fig:graph_time_cookie} 
\end{subfigure} 
  \caption{Time to digitize a sample is dependent on the sample's surface area and the desired final resolution. Cores benefit from linear surface area to sample length. The range of samples included span from a 3 mm $\times$ 220 mm core to a 75 mm $\times$ 56 mm cookie. The theoretical maximum sample dimensions are limited by the working plane of the machine, approximately 500 mm $\times$ 500 mm in our configuration. True sampling times are drastically influenced by configurable machine parameters such as percentage image overlap.} 
