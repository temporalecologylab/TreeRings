...
The general guidelines used for preparing cookies and cores applies---requiring samples to be sanded with incrementally increasing sand paper grit. Samples must be uniformly sanded and have a planar surface for scanning \citep{speer_fundamentals_2010} \R{line_change_2}.
...
% emwAug18 - why did we edit this so much? I am fine with it, just trying to understand the issue so I can make sure we addressed it....
...
\R{line_change_4} Among the over two dozen species (gynosperms and angiosperms) that we have scanned, we found the sharpness metric did not vary significantly when in-focus, allowing multiple similar species to be scanned in the same batch. % emwAug18 - can I comment out the end here? I am not sure what we mean and it does not seem critical to include % with only one core-edge threshold.
