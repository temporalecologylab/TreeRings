...
More recent techniques have aimed to achieve higher resolutions and/or larger samples through a different digitization approach---using an image-capturing system. These systems rely on a 
% emwAug18 - BELOW: is it really a microscrope camera? Maybe just say 'digital camera' ... or even be more general if needed and define DSLR while you're at it: camera (e.g., using a microscope camera or digital single-lens reflex---DSLR---camera)
microscope camera to take images, motors to move the sample or camera, a computer to manage the capturing, and a second, more powerful, computer to stitch images into a larger composite image.
...
% emwAug18 - I added in some commas for you below ... though see here which backs up my use of spaces! https://www.themathdoctors.org/decimals-commas-thousands-and-lakhs/
...
% emwAug18 - maybe this figure caption is the best place to cite the one of the hi-res images in the main text and provide a complete list of them in the supp PDF. 
\begin{figure}
  \centering
  ...
  \caption{Complete assembly of TIM, shown with a few smaller cookies ready to be scanned.  All of the 3D printed parts are available for download on the \href{https://doi.org/10.60705/3DPX/21561.3}{NIH 3D printing repository} and the software is  open source, with instructions to recreate this tool available on \href{https://github.com/temporalecologylab/TreeRings}{GitHub}. } % emwAug10 -- In addition to putting the links here, I would make sure each is sprinkled in around where most of the info related to it is introduced in the main text. 
% emwAug18 -- at the end you could add: Example images produced by TIM are provided in the SI. 
% emwAug18 -- In the supp I might make a TABLE of the images (3 cols: file name, species and description) and you could just link to that table here... 
...
% emwAug18 - I made some edits based on what CRD told me he will send ... 
\R{line_change_4} Among the over two dozen species (gynosperms and angiosperms) that we have scanned, we found the sharpness metric did not vary significantly when in-focus, allowing multiple similar species to be scanned in the same batch.
...
If the new image has at least a sharpness metric of 95\% of its previously imaged neighbor, we choose not to run the autofocusing procedure. If the prepared surface of the core is perfectly planar to the table that TIM is installed on, we have found that it is possible to only autofocus once for tens of images while maintaining high image sharpness (REF SUPP TABLE). % emwAug18 - perfect place to ref that table of images in the supp that we'll add! I would mention it here and in the caption....
...
% emwAug18 - moved down, see what you think and make sure my edits are correct ... 
...
\R{autofocusing_2} %% End reviewer comments 1
...
The file sizes of entire cookie scans can quickly become too large to analyze with commonly available software. A full resolution scan (21,140 DPI) of a 10 cm x 10 cm cookie is multiple gigabytes in a lossless file format. 
...
% emwAug18 - I am bit lost on what we're trying to say below ... I will paste to github and hopefully we can fix it there.
Sanding samples in bulk inherently led to a few outlier samples having blemishes or imperfections on the scanning surface, thus we developed a bench-top method to circumvent the need to resand these samples. %  for logistical convenience. 
Additionally, this method aids stitching images in regions of few distinguishable features despite effective sanding, primarily in regions of lightly-colored sapwood. % Betula / Populus?  
For such cases, we developed a particle smearing method which embeds particles in the prepared surface of the sample, thus aiding the stitching process.
Particle smearing does not need to be done to every sample and is best reserved for samples that have proven to be difficult to stitch. We found this method did not interfere with the measurements of annual growth rings. Measurement of other anatomical measurements such as vessel counting will likely be affected. 
\R{particle_smearing_2}
% End reviewer comments 5
...
We found that particle smearing can be an effective technique for adding high contrast features to the prepared surface of a sample by embedding small particles into the surface of the sample. 
...
\R{llm_rewording_1}
We wrote all the initial code for our first fully functioning prototype version of TIM, % emwAug18 -- maybe give the years: We wrote all the initial code for our first fully functioning prototype version of TIM (in 2021-2022) or whatever they were... 
...
Other small snippets of code were generated to select pixels in specific regions of images. We carefully reviewed all generated code for functionality, and note each use of it clearly with comments in the source files. 
\R{llm_rewording_2}
...
% emwAug18 - I moved around the usage to two places, see what you think...
TIM is capable of producing extremely high resolution scans across diverse species. \R{usage_1}To date, we have used TIM to scan over 1,200 cores and a 100 cookies spanning XX species (REF TABLE in supp)  \R{usage_2}
We calculated the maximum dots per inch (DPI) at 21,140 of individual images captured from the system using a microscopy slide scale (with 0.01 mm graduations, Figure \ref{fig:dpi_measurements}), and developed a Python tool to replicate this for varied lens focal lengths. When downscaling the final stitch, 
DPI scales linearly with the downscale percentage defined in the machine parameters. % emwAug18 - make sure the \ref{fig:dpi_measurements} is correct, showing up as ??
...
% emwAug18 - I would change the below to; TIM can scan cores up to 1.5 m in length, which is 50\% larger than the longest increment borer available on the market (though a larger ACRO frame than shown here would be needed to support such longer sample lengths). 
\R{max_length}Cores can be scanned at maximum resolution up to 1 meter which can be collected from the longest increment borer available on the market (though a larger ACRO frame than shown here would be needed to support this sample length). % emwAug18 - be sure your \ref{}are against the text on one side, otherwise you add white space
...
\begin{figure}[htbp]
  \todo{Make this figure fit better and fill with real data.}
  \centering
  \begin{subfigure}{.5\textwidth}
    \centering
    \includegraphics[height=\linewidth]{../graphs/core_scanning_time.png}
    \caption{Core Scanning Time}
    \label{fig:graph_time_core}
  \end{subfigure}%
  \begin{subfigure}{.5\textwidth}
    \centering
    \includegraphics[height=\linewidth]{../graphs/cookie_scanning_time.png}
