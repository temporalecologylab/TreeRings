...
\begin{figure}
  ...
  \caption{Complete assembly of TIM, shown with a few smaller cookies ready to be scanned.  All of the 3D printed parts are available for download on the NIH 3D printing repository (HYPERLINK REMOVED) and the software is  open source, with instructions to recreate this tool available on GitHub (HYPERLINK REMOVED). } % emwAug10 -- In addition to putting the links here, I would make sure each is sprinkled in around where most of the info related to it is introduced in the main text. 
...
This maximum cookie diameter is significantly smaller than the maximum core sample length. \R{max_length}TIM can scan cores up to 1.5 m in length, which is 50\% larger than the longest increment borer available on the market (though a larger ACRO frame than shown here would be needed to support such longer sample lengths).  % emwAug18 - be sure your \ref{}are against the text on one side, otherwise you add white space
